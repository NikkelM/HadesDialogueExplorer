...
// Populated in :func:`init` from the loaded dataset and iterated in
// :func:`searchTextLines`.
let linesIndex;
...
// from :func:`initSearch` so text and name search can share the same
// tokeniser and so the search code path stays small enough to read at
// a glance.
function searchNameMatches(tokens, limit) {
    const ranked = [];
...
// Called once during :func:`init`. Storing the lowercased text once
// up-front avoids re-lowercasing on every keystroke.
function buildLinesIndex() {
    const out = [];
...
function findWordPositions(textLower, token) {
    const positions = [];
...
// already satisfied.
function findContiguousPhrasePosition(textLower, phraseTokens, firstTokenPositions) {
...
function searchTextLines(tokens, excludeNames, limit) {
...
// ``data-name`` attribute consumed by :func:`initSearch`'s click
// handler.
function renderTextMatchHtml(match, tokens) {
...
function buildSnippetHtml(textOriginal, tokens, positionsByToken, anchorPos) {
    const matches = [];
